                ...
                <div className="site-nav-inside">
                  ...
                  <ul className="menu">
                    ...
                    <li key={action_idx} className={classNames('menu-item', {'current-menu-item': _.get(this.props, 'pageContext.url', null) === _.get(action, 'url', null)})}>
                      <Link to={safePrefix(_.get(action, 'url', null))}{...(_.get(action, 'new_window', null) ? ({target: '_blank', rel: 'noopener'}) : null)}>{_.get(action, 'label', null)}</Link>
                    </li>
                    ))}
                  </ul>
                  )}
                  ...
                  <div className="social-links">
                    ...
                    action && (
                    <Link key={action_idx} className={classNames({'button-circle': _.get(action, 'style', null) === 'icon'})} to={safePrefix(_.get(action, 'url', null))}{...(_.get(action, 'new_window', null) ? ({target: '_blank', rel: 'noopener'}) : null)}>
